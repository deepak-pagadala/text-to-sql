...
ROW_THRESHOLD = 500          # tweak as needed
TOP_K_DEFAULT = 10
...
# ------------------------------------------------------------------- #
#  DEBUG SWITCH  –  True to inspect SQL strings #
# ------------------------------------------------------------------- #
DEBUG_SQL = False          # ← set False for normal runs
...
""".strip()
    ...
    return sql_lines[:3]
# ------------------------------------------------------------------- #
#  BASIC BROAD-QUERY HEURISTICS  (run **before** any LLM call)        #
# ------------------------------------------------------------------- #
TOP_K_DEFAULT  = 10                       # rows we propose to the user
BROAD_KEYWORDS = (
...
def _count_rows(conn: sqlite3.Connection, sql: str) -> int:
    ...
    try:
        ...
        # last-chance fallback: iterate manually but cap at one million rows
        try:
...
def execute_sql_query(sql_query: str, db_path: str = DB_PATH):
    ...
    """
    # ── safety: drop everything after first semicolon ─────────────── #
    if ";" in sql_query:
...
# ------------------------------------------------------------------- #
# 3.  STRIP MARKDOWN FENCES  **and extra statements**  FROM SQL       #
# ------------------------------------------------------------------- #
def clean_sql_query(query: str) -> str:
    ...
    # keep text before the first semicolon, ignoring any trailing comments
    if ";" in q:
        q = q.split(";", 1)[0]
...
# ------------------------------------------------------------------- #
#  APPEND (QUESTION, SQL) TO sample_query.csv + REFRESH EMBEDDINGS    #
# ------------------------------------------------------------------- #
...
def run_pipeline(user_query: str, db_path: str = DB_PATH):
    ...
    conn = sqlite3.connect(db_path)          # one connection for fast COUNTs
    try:
        for sql in candidate_sql:
            cleaned = clean_sql_query(sql)
            raw_sql = revert_aliases_in_query(cleaned)

            # ── NEW:  estimate result size and reframe if too large ───── #
            ...
            if row_cnt > ROW_THRESHOLD:
                ...
                if ans == "y":
                    # regenerate SQL for the reframed prompt
...
# ------------------------------------------------------------------- #
#  INTERACTIVE CLI  –  with pre-reframe safety gate                   #
# ------------------------------------------------------------------- #
def interactive_cli():
    """
    ...
    while True:
        ...
        # C)  user picked “None”  →  breadth check -------------------- #
        if is_broad_query(user_q):
            ...
            if ans == "1":
                user_q = alt_q            # proceed with reframed query
            else:
                print("Okay, please try a different question.")
                continue                  # restart loop – *no* LLM
        # D)  safe: run full pipeline --------------------------------- #
        try:
