...
from transformers import (
    ...
    AutoTokenizer,
    BitsAndBytesConfig,            # still here if you add 4-bit later
)
...
# -------------------------------------------------------------------------- #
# 2.  LOAD LLAMA-2 MODEL & TOKENIZER
# -------------------------------------------------------------------------- #
...
llama_model = AutoModelForCausalLM.from_pretrained(
    LLAMA_MODEL_ID,
    device_map="auto",             # still works; needs accelerate installed
    torch_dtype=torch.float16,
    token=HF_TOKEN,
)

def llama_generate(prompt: str, max_new: int = 256) -> str:
    """
    Generate a response with Llama-2 and return it as plain text.
    """
    wrapped  = f"<s>[INST] {prompt} [/INST]</s>"
    inputs   = tokenizer(wrapped, return_tensors="pt").to(DEVICE)
    outputs  = llama_model.generate(**inputs, max_new_tokens=max_new)
